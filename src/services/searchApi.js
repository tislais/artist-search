...
export const fetchArtists = async (search, page) => {
  const res = await fetch(`https://musicbrainz.org/ws/2/artist?query=${search}&fmt=json&limit=100`);
  const json = await res.json();
  return json.artists.slice((page - 1) * 10, page * 10);
}

export const fetchReleases = async (id, page) => {
  const res = await fetch(`https://musicbrainz.org/ws/2/release?artist=${id}&fmt=json`);
  const json = await res.json();
